# Scenario 003 — Detection, association and tracking over the X-band scenario

> **Reading order.** This document follows the standard scenario scaffolding — Status & Purpose,
> Overview & Geometry, Parameter Matrix, Core Decisions, Build Sequence, Acceptance Criteria — but
> the numbered sections inside it keep the numbers they were written with, because §14 refers to
> them by number and that record is the most useful thing here. The six parts below are a
> reordering, not a renumbering.
>
> | Part | Sections |
> | :--- | :--- |
> | 1 — Status & Purpose | §1 |
> | 2 — Overview & Geometry | the two pipeline diagrams, §4 |
> | 3 — Parameter Matrix | §2, the mathematical contracts, §5, §9 |
> | 4 — Core Decisions | §3, §6, §7, §8, §11, §13 |
> | 5 — Build Sequence | §10 |
> | 6 — Acceptance Criteria | §12, the verification matrix, §14 |

---

# Part 1 — Status & Purpose

Status: **implemented**. The second vertical slice, stacked on
`spec/scenario-001-xband.md`. Two scenarios ship:
`scenarios/scenario_003_tracking.toml` over S1 and
`scenarios/scenario_003_tracking_dual_prf.toml` over S3.

Where the implementation departs from what follows, §14 says so and wins. The
body below is kept as written, because several of its predictions turned out to
be wrong in instructive ways and the record of what was expected is worth more
than a tidied-up document.
...
3. It produces two more pictures: truth and detections overlaid on the RD map, and a track
   plotted against time and truth. Both are things an intern can look at and immediately say is
   wrong.
...
# Part 2 — Overview & Geometry

## Overview: the two pipelines this scenario adds

Everything up to the range-Doppler map is scenario 001's S1 (or S3) path, unchanged. What is new
begins at the map.
...
```
  RangeDopplerProduct  (rd_map, range_axis_m, velocity_axis_mps)
            |
            v
  core/detection.py   cfar_detect (CA, axis = -1)  +  cfar_valid_mask
            |             pfa = 1e-5, n_train = 16, n_guard = 4, alpha = 11.417 dB
            v
            boolean detection map, (256, 1000)
            |
            v
  pipelines/tracking.py  roll to the quietest Doppler row   <-- §14.2, the wrap
            |
            v
  core/detection.py   cluster_detections -> Detection(peak_index,
            |             centroid_index, peak_power_w, total_power_w, n_cells)
            v
  keep only the strongest detection per range cell   <-- §14.3, the sidelobes
            |
            v
  pipelines/tracking.py  fractional index -> metres and m/s, via
            |             dsp.range_bin_centers_m / dsp.doppler_bin_centers_mps
            v
  velocity unfolding: track_aided | oracle | none    <-- §5.3
            |
            v
...
```
                       unassociated measurement
                                ...
                                v
                        +----------------+
                        |   TENTATIVE    |  dim 1, range only (§5.3 bootstrap)
                        |  P0 rate var   |  seeded from 1 row only (§14.5)
                        |   = v_max^2    |
                        +----------------+
                         |             |
          ...
          |         |                               | forget rate, |
          |         v                               | widen gate   |
          |  batch range-slope sigma < v_span/6     +--------------+
          |  (about 10 frames)                             |
          |         |                                      | hit
          |         v                                      |
...
| Unambiguous range | 37.474 km — the target never folds in range |
...
| Slant range, full track | 5.326 – 22.010 km |
| Slant range, this scenario's window | 18.16 – 21.47 km |
| Default window | 120 s = 120 frames, from `start_time_s = 663.0` — see §14.10 |
...
Scenario 001 §3.5 holds unchanged for everything upstream of the range-Doppler map. What this
scenario adds:

| Contract | Statement |
| :--- | :--- |
| Detection map | `(n_doppler_bins, n_range_bins)` bool — `(256, 1000)` for S1 |
| `cfar_valid_mask` | Same shape; `245_760` of `256_000` cells True at `n_train = 16`, `n_guard = 4`, `axis = -1` |
| `Detection.centroid_index` | **Fractional** cell indices, power-weighted; interpolated into the bin-centre axes, never converted with a locally derived bin spacing |
| Measurement `z` | `(dim,)` float64, with `dim` in `{1, 2, 3, 4}` per §5.2 and §5.3 — it **varies within a run** |
| Measurement noise `R` | `(dim, dim)` float64, diagonal; `diag(sigma_range_m², sigma_velocity_mps²)` at `dim = 2` |
| State `x`, covariance `P` | `(n_state,)` and `(n_state, n_state)`; `n_state` = 2 (`range_1d`), 4 (`enu_2d`), 6 (`enu_3d`) |
| `state_model_matrices(...)` | Returns `(F, Q, h, H_jac, R, dim)` — **data, not a class hierarchy**, per §6.2 |
...
§2's table is what was specified. These are the values the two TOMLs actually carry, with the
section that records each departure:

| Key | `scenario_003_tracking.toml` | `..._dual_prf.toml` | Note |
| :--- | :--- | :--- | :--- |
| `[trajectory].start_time_s` | 663.0 | 663.0 | §14.10 — not the 0.0 scenario 001 uses |
| `[trajectory].duration_s` | 120.0 | 120.0 | |
| `[detection].pfa` | 1e-5 | 1e-5 | §3 |
| `[detection].n_train`, `n_guard` | 16, 4 | 16, 4 | per side |
| `[detection].variant` | `ca` | `ca` | |
| `[detection].merge_range_bins` | 1.0 | 1.0 | §14.3 — strongest detection per range cell |
| `[tracking].state_model` | `range_1d` | `range_1d` | §6 |
| `[tracking].sigma_range_m` | 21.635653 | 21.635653 | §5.4, `74.9481/√12` |
| `[tracking].sigma_velocity_mps` | 0.01724781 | 0.01724781 | §5.4, `0.0597482/√12` |
| `[tracking].sigma_azimuth_deg` | 0.5 | 0.5 | §6.3, the simulated angle |
| `[tracking].sigma_accel_mps2` | **5.0** | **5.0** | §14.6 — not §6.2's 2.0 |
| `[tracking].gate_probability` | 0.99 | 0.99 | §7 |
| `[tracking].n_confirm_hits` / `n_confirm_frames` | 4 / 5 | 4 / 5 | §7 |
| `[tracking].n_delete_misses` | 3 | 3 | §7 |
| `[tracking].n_reacquire_frames` | 5 | 5 | §14.7 — re-acquisition, added after the fact |
| `[tracking].v_max_mps` | 200.0 | 200.0 | §6.2's `P₀` |
| `[tracking].velocity_unfolding` | `track_aided` | **`none`** | §5.3; S3 resolves it in the waveform |
| `[tracking].unfold_sigma_gate` | 6.0 | 6.0 (unused) | §5.3 |
| `[tracking].n_slope_frames` | 5 | 5 (unused) | §14.4 — the batch range-slope estimator |
| `[tracking].simulated_angles` | false | false | §6.3, §14.9 |

The Python field for `velocity_unfolding` is `unfolding_mode`; `configs_from_scenario` is the single
place the two spellings meet, and §14.8 records why.
...
- range, from `dsp.range_bin_centers_m(n_bins, bandwidth_hz, chirp_duration_s, sample_rate_hz)`,
...
| `track_{frame:05d}.png` | Range against time: truth trajectory, detections so far, track history, current gate |
...
until post-integration SNR at 21.47 km sits a few dB above the 11.417 dB threshold factor puts Pd
...
### 13.5 Two-dimensional clustering — a note

Added by §14.2. `cluster_detections` should grow a `wrap_axes` argument naming
the axes that are circular, so a target on the Doppler wrap is labelled as one
...
§12 in one table, with the bound each criterion asserts and where it is checked. §14.7 records
what was measured against it.

| # | Criterion | Bound | Verified by |
| :--- | :--- | :--- | :--- |
| 1 | One confirmed track, stable `track_id` | present in >= 95 % of frames from first confirmation; id count 1 | `tests/pipelines/test_scenario_003.py` |
| 2 | Range accuracy | RMSE < 74.9481 m (one range bin) | same |
| 3 | Range-rate accuracy, post-bootstrap | RMSE < 2 m/s | same |
| 4 | Fold index correct in every confirmed unfolded frame; a mis-unfold shows as a gate rejection and a miss, never an accepted update | exact match against `truth.csv`; both halves asserted | same |
...
since M-of-N cannot confirm before its fourth hit.

## 14. What building it changed

Everything above was written before the scenario ran. Most of it survived: §3's
false-alarm arithmetic, §5.4's measurement noise, §7's gate thresholds and
§5.1's bin-centre seam all reproduced exactly against the code. Seven things did
not, and they are recorded here rather than edited into the body, because the
gap between the two is the most useful thing this document now contains.

### 14.1 The state's sign convention was self-contradictory

§5.2 defines the measurement as positive closing, per `structure.md` D5, and §6
pairs it with a state `[r, ṙ]`, a transition `[[1, T], [0, 1]]` and `H = I₂`.
Those three cannot all hold: with that transition a closing target's range
*grows*. The implementation keeps the measurement convention and gives
`range_1d` a transition carrying `−T`, with the process-noise cross terms
changing sign to match — the textbook constant-velocity pair conjugated by
`diag(1, −1)`.

This is the one error here that produced a plausible picture. The filter dead
reckoned the target the wrong way down the line of sight, every fold selected
from its prediction was wrong, and the range track still looked roughly right
for several frames. `tests/core/test_tracking.py` pins it.

### 14.2 The Doppler axis is circular and `cluster_detections` is not

§5.1 assumes a detection cluster is a cluster. But a target straddling the
±7.65 m/s wrap is labelled as *two* clusters at opposite ends of the Doppler
axis, and the power-weighted mean of the pair is nowhere near the target. At S1
frame 0 the split is 58 cells at +7.493 m/s and 37 at −7.511 m/s, against a true
folded velocity of +7.502 m/s.

`pipelines/tracking.py` rolls the map so its array edge falls on the quietest
Doppler row before clustering, then maps the indices back. The clean fix is a
`wrap_axes` option on `cluster_detections` itself — see §13.5.
...
one-cell satellites 10–28 Doppler bins away, while genuine false alarms land
16–750 *range* bins off.

Only the strongest detection per range cell is kept. One point target cannot
produce two returns at one range. This is a single-target simplification of
exactly the kind §13.2 and §13.3 describe.

### 14.4 The bootstrap threshold is unreachable from the filter's covariance

§5.3 proposes promoting a track out of its range-only bootstrap once
`sqrt(P[range_rate, range_rate]) < v_span / 6` = 2.55 m/s, and derives the
ten-frame crossing from a least-squares range slope. Those are two different
estimators and only one of them works. Discrete white-noise acceleration adds
`σ_a²T²` to the rate variance every frame, so at `sigma_accel_mps2 = 2.0` the
filter's rate standard deviation floors at about 4.1 m/s and never reaches 2.55:
the track would stay range-only for ever.

The batch estimator §5.3 actually computes is not floored this way, reproduces
the 6.84 m/s and 3.34 m/s the section quotes, and crosses the threshold at
N = 10 exactly as predicted. That is the one used. The floor is pinned by a test
so that if it ever drops, the simpler covariance test can replace it.

### 14.5 A new track must not be seeded with a folded velocity

§7 asks for single-point initiation "once a track is unfolded", and for a new
track to be seeded with `P₀`'s `v_max²` rate variance. It is easy to read the
...
§6.2 picks 2.0 by judgement, for a light aircraft's lateral acceleration, and
says so honestly. But the target is synthesised from
`data/flight_coordinates.csv`, whose fixes are 2–3 s apart and carry ADS-B
position noise. The *simulated* target's range rate therefore moves 3.4 m/s
between frames (standard deviation over the default window) and by as much as
11 m/s. The filter has to track the target as simulated, not as flown.

**Re-measured, the margin is gone.** The 98%-against-93% figure was taken before
§14.10 moved the default window to 663 s and before §14.7 added re-acquisition.
Over the current 120-frame window both settings hold a confirmed track in 117 of
120 frames under 2 track ids, and 2.0 selects the fold in 94% of associated
frames against 90% at 5.0. Re-acquisition holds a confirmed track through a
mis-unfold, which is what makes retention insensitive to `sigma_accel_mps2`
...
This is the substantive one. Track-aided unfolding must predict the range rate
to better than half a fold span, 7.65 m/s. Per §14.6 the simulated target's rate
moves by up to 11 m/s between frames, and exceeds half a span in **3.4%** of
frames. No selector can do anything about those: the information is not present.

Worse, a mis-unfold is self-consistent. The measurement agrees perfectly with
the wrong prediction that chose it, so it passes the gate, and with a velocity
variance five orders of magnitude below the range variance the accumulating
range residual can never pull the estimate back. §5.3 is right that a mis-unfold
*ought* to be rejected as a miss; what it misses is that once a track has
committed to a wrong fold, the mis-unfold is no longer the measurement that
disagrees with it.

Three mechanisms were added in response, all measured:

| Mechanism | Effect over 120 frames |
| :--- | :--- |
| Fold-consistency monitor against the unambiguous range history | fold selection 82% → 86% |
| Range-only fallback when a measurement fails only the velocity gate | 93% → 97% of frames tracked |
| Re-acquisition: a confirmed track forgets its rate and widens its gate instead of being deleted | 6 track ids → 2 |
...
| Criterion | §12 asks | S1 | S3 dual-PRF |
| :--- | :--- | :--- | :--- |
| 1 — one confirmed track, stable id | ≥95% of frames, one id | 98%, 2 ids | 98%, **1 id** |
| 2 — range RMSE | < 74.95 m | **17.5 m** | **2.0 m** |
| 3 — range-rate RMSE | < 2 m/s | 5.8 m/s | **0.60 m/s** |
| 4 — fold index correct | every frame | 86% | **every frame** |
...
§12's criteria are otherwise unchanged, with two adjustments the acceptance
tests document: they run 20 frames rather than 15, because the bootstrap holds a
track range-only for ten frames and S1's first fold change is at frame 17; and
criterion 1 is measured from the first confirmation, since M-of-N cannot confirm
before its fourth hit and those frames are 15% of a 20-frame window.

### 14.8 Smaller things

- **This document called the track scope a "plan view" and it never was one.** §9 and
  `structure.md` both used the term; a plan view is the ground plane, and §4's radar measures
...
- `FrameTracks` stores *snapshots* of its tracks. A `Track` is mutable by
  ...
  wrong in a way that still looked like a track.
- The TOML key is `velocity_unfolding`, as §2 says, but `scripts/check_conventions.py`
  rejects a *parameter* whose leading token is a bare unit name. The Python
  field is `unfolding_mode`; `configs_from_scenario` is the single place the two
  spellings meet.
- pytest moved to `--import-mode=importlib`: tests mirror `src/` per
  `docs/conventions/testing.md` §1, so `tests/core/test_tracking.py` and
...
- **Criterion 7 end to end.** The ENU state models, their Jacobians and the
  agreement between `enu_2d` and `range_1d` are implemented and tested at unit
  level, including a central-difference check of both Jacobians. What is not
  built is §6.3's simulated angle measurement in the pipeline, so criterion 7 has
  not been run over a whole scenario. The unit test covers the Jacobian question
  it exists to answer; the scenario-level version is outstanding.
- **§13.1's lower link budget**, which remains the right next slice: at the
  default 100 W the target is detected in every frame, so §7's coast-and-delete
  logic is exercised only by mis-unfolds and never by a genuine missed detection.
...
crosses a fold boundary -- the thing this scenario exists to show -- while holding the rate steady
enough either side of the crossing for the batch range-slope estimator of §14.4 to resolve it.
Over that window the target runs 18.16 to 21.47 km and the radial rate -39.6 to -14.7 m/s.
