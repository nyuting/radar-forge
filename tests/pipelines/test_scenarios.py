...
from radar_forge.core.tracking import state_model_matrices
...
class TestLoadScenario:
    ...
    def test_bad_physics_is_reported_by_the_radar_not_the_loader(self, tmp_path: Path) -> None:
        ...
        broken.write_text(
            S1_TOML.read_text().replace("chirp_duration_s = 1.0e-3", "chirp_duration_s = 2.0e-3")
        )
...
class TestSpecifiedAmbiguities:
    ...
    def test_s3_burst_b_runs_at_exactly_one_over_six_thousand_seconds(self) -> None:
        ...
        np.testing.assert_allclose(burst.transmitter.chirp_duration_s, 1.0 / 6000.0, rtol=1e-15)
...
class TestScenarioValidation:
    ...
    def test_rejects_a_scenario_with_no_burst_at_all(self) -> None:
        """A scenario with nothing to transmit is not a degenerate run, it is a typo.

        The loader catches a TOML with no [[burst]]; this catches the same
        state reached in code, which is how a caller building a Scenario
        programmatically would hit it.
        """
    ...
    def test_rejects_a_burst_with_no_pulses(self, n_pulses: int) -> None:
        ...
        Left unguarded it produces an empty slow-time axis, and every transform
        downstream accepts it, so the run completes and simply never detects
        anything.
        """
        with pytest.raises(ValueError, match="strictly positive n_pulses"):
            replace(load_scenario(S1_TOML), n_pulses=(n_pulses,))

    def test_rejects_a_non_positive_frame_rate(self) -> None:
        """Frames per second sets the filter's time step; zero divides by zero."""
        with pytest.raises(ValueError, match="frame_rate_hz"):
            replace(load_scenario(S1_TOML), frame_rate_hz=0.0)


class TestDegenerateWindows:
    """Windows the frame grid cannot difference into a radial velocity."""

    def test_a_single_frame_with_no_room_to_pad_is_rejected(self) -> None:
        """Radial velocity is a difference, so one sample is not enough geometry.

        `iterate_frames` pads the frame grid by one interval on each side so
        that even a one-frame window can be differenced. When the frame
        interval is longer than the whole track there is nowhere to pad, and
        the alternative to raising is a velocity of nan propagating into the
        Doppler truth label.
        """
        scenario = load_scenario(S1_TOML)
        trajectory = load_flight_csv(
            scenario.trajectory_path, altitude_m=scenario.target_altitude_m
        )
        longer_than_the_track_s = trajectory.duration_s + 10.0
        degenerate = replace(
            scenario,
            start_time_s=0.0,
            frame_rate_hz=1.0 / longer_than_the_track_s,
            duration_s=longer_than_the_track_s,
        )
        assert degenerate.n_frames == 1
...
class TestBurstRangeDoppler:
    ...
    def test_the_pulsed_range_axis_spans_one_unambiguous_range(self) -> None:
        ...
        burst = scenario.bursts[0]
        product = form_range_doppler_map(next(iter(iterate_frames(scenario))).iq[0], burst)
        np.testing.assert_allclose(product.range_axis_m[0], 0.0, atol=1e-12)
    ...
    def test_a_synthetic_fmcw_target_lands_at_its_true_range_and_velocity(self) -> None:
        ...
        peak_range_m, peak_velocity_mps = peak_range_velocity(form_range_doppler_map(cube, burst))
...
class TestBistaticScenarios:
    ...
    def test_the_baseline_is_the_specified_airport_to_receiver_distance(self) -> None:
        """Checked against spec/scenario-002-bistatic.md S2."""
        pair = load_scenario(B1_TOML).bursts[0]
        assert isinstance(pair, BistaticRadar)
        np.testing.assert_allclose(pair.baseline_m, 19_601.4, atol=5.0)
    ...
    def test_the_two_variants_differ_only_in_the_carrier(self) -> None:
        ...
        for field in ("bandwidth_hz", "transmit_power_w", "chirp_duration_s", "prf_hz", "waveform"):
    ...
    def test_rejects_an_unknown_key_in_the_transmitter_site(self, tmp_path: Path) -> None:
        ...
        broken = tmp_path / "broken.toml"
        broken.write_text(
            B1_TOML.read_text().replace("latitude_deg = 35.87750", "lat_deg = 35.87750")
        )
        with pytest.raises(ValueError, match="unknown key"):
            load_scenario(broken)
...
class TestScenario003StateModelParameters:
    """Refactor-001 §3.2: both scenario 003 TOMLs carry the ``enu_2d`` sigmas."""

    @pytest.mark.parametrize(
        "toml_path",
        [
            SCENARIOS_DIR / "scenario_003_tracking.toml",
            SCENARIOS_DIR / "scenario_003_tracking_dual_prf.toml",
        ],
        ids=["s1", "dual-prf"],
    )
    def test_the_enu_2d_parameters_load_and_build_a_model(self, toml_path: Path) -> None:
        """Present in the file, carried into the config, and usable by the model."""
        scenario = load_scenario(toml_path)
        assert scenario.tracking_table is not None
        assert scenario.tracking_table["sigma_azimuth_deg"] == 0.5
        _, tracking = configs_from_scenario(scenario)
        assert tracking.state_model == "range_1d"
        assert tracking.sigma_azimuth_deg == 0.5

        # The point of carrying them: enu_2d builds from this TOML alone.
        model = state_model_matrices(
            "enu_2d",
            1.0,
            sigma_accel_mps2=tracking.sigma_accel_mps2,
            sigma_range_m=tracking.sigma_range_m,
            sigma_velocity_mps=tracking.sigma_velocity_mps,
            sigma_azimuth_deg=tracking.sigma_azimuth_deg,
        )
        assert model.measurement_dim == 3
        assert model.transition.shape == (4, 4)
        # The azimuth row of R is the TOML's sigma, in radians squared.
        assert model.measurement_noise[1, 1] == pytest.approx(np.deg2rad(0.5) ** 2)
