...
class Transmitter:
    ...
    ------
    ValueError
        If any quantity is non-positive, or if ``chirp_duration_s * prf_hz`` exceeds
        one, which would mean a chirp had not finished before the next began.

    Notes
    -----
    A 100% duty cycle (``chirp_duration_s * prf_hz == 1``) is permitted and is the
    ...
    def __post_init__(self) -> None:
        ...
        if self.duty_cycle_linear > 1.0:
            msg = (
                f"chirp_duration_s * prf_hz = {self.duty_cycle_linear:.4f} exceeds 1.0: the next "
    ...
    def duty_cycle_linear(self) -> float:
        ...
        return self.chirp_duration_s * self.prf_hz

    @property
    def sweep_rate_hzps(self) -> float:
        r"""Chirp sweep rate :math:`\alpha = B/T`, Hz/s."""
        return float(sweep_rate_hzps(self.bandwidth_hz, self.chirp_duration_s))
...
class Radar:
    ...
    def n_samples_per_chirp(self) -> int:
        ...
        """
        return round(self.transmitter.chirp_duration_s * self.receiver.sample_rate_hz)
