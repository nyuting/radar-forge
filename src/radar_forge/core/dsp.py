...
def matched_filter(
    ...
    *linear* convolution. A hand-written ``ifft(fft(x) * conj(fft(h)))`` computes
    ...
    >>> chirp = lfm_chirp(bandwidth_hz=10e6, chirp_time_s=10e-6, sample_rate_hz=20e6)
...
def range_bin_centers_m(
    ...
    --------
    >>> import numpy as np
    >>> bins = range_bin_centers_m(8, bandwidth_hz=1e9, chirp_time_s=40e-6, sample_rate_hz=1e6)
    ...
    result: NDArray[np.float64] = range_from_beat_frequency_m(beat_hz, bandwidth_hz, chirp_time_s)
    return result
...
def mti_filter(
    ...
    *,
    ...
    # convolution would introduce at both ends of the dwell.
    coefficients = _MTI_COEFFICIENTS[n_pulses]
    n_output = n_available - (n_pulses - 1)
    accumulator = np.zeros_like(np.take(samples_arr, range(n_output), axis=axis))
    for offset, coefficient in enumerate(coefficients):
        taps = np.take(samples_arr, range(offset, offset + n_output), axis=axis)
        accumulator = accumulator + coefficient * taps
