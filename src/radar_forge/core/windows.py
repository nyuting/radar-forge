...
"""The taper shapes :func:`taper` accepts, in order of increasing sidelobe suppression.

A :class:`~typing.Literal` rather than a bare ``str``, matching
:data:`radar_forge.core.detection.CfarVariant`, so a misspelled taper is caught
by ``mypy`` at the call site instead of raising at run time.
"""

TAPER_NAMES: tuple[TaperName, ...] = get_args(TaperName)
"""The taper names as a tuple, for iterating or for a run-time membership test.

``taylor`` and ``chebyshev`` are parametric: their sidelobe level is set by the
``sidelobe_db`` argument rather than fixed by the window shape. Derived from
:data:`TaperName` rather than retyped, so the two cannot drift apart.
"""
...
def taper(
    ...
    *periodic* variant, one sample shorter and wrapped, is for spectral density
    ...
    Parameters
    ----------
    name : {'rectangular', 'hann', 'hamming', 'blackman', 'blackmanharris', \
'taylor', 'chebyshev'}
