...
def sweep_rate_hzps(bandwidth_hz: ArrayLike, chirp_time_s: ArrayLike) -> NDArray[np.float64]:
    ...
    """
    bandwidth_arr = np.asarray(bandwidth_hz, dtype=np.float64)
    chirp_time_arr = np.asarray(chirp_time_s, dtype=np.float64)
    if np.any(bandwidth_arr <= 0.0):
        msg = "bandwidth_hz must be strictly positive."
        raise ValueError(msg)
    if np.any(chirp_time_arr <= 0.0):
        msg = "chirp_time_s must be strictly positive."
        raise ValueError(msg)
    result: NDArray[np.float64] = bandwidth_arr / chirp_time_arr
    return result
...
def beat_frequency_hz(
    ...
    --------
    >>> import numpy as np
    >>> f_b = beat_frequency_hz(range_m=42.0, bandwidth_hz=1e9, chirp_time_s=40e-6)
    ...
    result: NDArray[np.float64] = sweep_rate_hzps(bandwidth_hz, chirp_time_s) * round_trip_delay_s
    return result
...
def range_from_beat_frequency_m(
    ...
    result: NDArray[np.float64] = (
        SPEED_OF_LIGHT_MPS * beat_arr / (2.0 * sweep_rate_hzps(bandwidth_hz, chirp_time_s))
    )
    return result
...
def lfm_chirp(
    ...
    *,
    ...
    ``chirp_time_s``, giving the analytic signal
    ...
    numpy.ndarray
        ...
        ``n_samples = round(chirp_time_s * sample_rate_hz)``.
    ...
    n_samples: int = round(chirp_time_s * sample_rate_hz)
    if n_samples < 1:
        msg = (
            f"chirp_time_s ({chirp_time_s:g}) is too short to yield a sample at "
    ...
    sweep_rate = bandwidth_hz / chirp_time_s
    if not up_sweep:
