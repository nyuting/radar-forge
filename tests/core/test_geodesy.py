...
# The scenario-001 radar site: DSO National Laboratories, 12 Science Park Drive.
DSO_LATITUDE_DEG = 1.29150
DSO_LONGITUDE_DEG = 103.78710
DSO_ALTITUDE_M = 60.0
...
class TestGeodeticToEnu:
    ...
    def test_is_a_rigid_motion_so_distance_is_preserved(self) -> None:
        ...
        latitude_deg = DSO_LATITUDE_DEG + rng.uniform(-0.1, 0.1, size=32)
        longitude_deg = DSO_LONGITUDE_DEG + rng.uniform(-0.1, 0.1, size=32)
        ...
        origin_ecef_m = geodesy.geodetic_to_ecef_m(
            DSO_LATITUDE_DEG, DSO_LONGITUDE_DEG, DSO_ALTITUDE_M
        )
...
def test_scenario_001_track_lies_where_the_spec_says() -> None:
    """The flight track is 8-18 km from the radar, per spec/scenario-001.
    ...
    assert np.all(range_m > 6_000.0)
    assert np.all(range_m < 25_000.0)
    # Both corners are north of the radar, so azimuth is northerly.
