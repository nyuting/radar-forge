...
| **Steinmetz Neural Networks** | Complex-valued neural networks (AISTATS '25) by the RASPNet author, with a worked RASPNet regression notebook. The model counterpart to the RASPNet row above. | Reference for the complex-valued I/Q feature convention in `pipelines/datasets.py` — features stay complex rather than split into two real channels — and, with RASPNet, a paired dataset-plus-model benchmark. Licence undeclared; no code vendored, never a dependency. |
