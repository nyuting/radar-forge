...
- [`spec/scenario-001-xband.md`](spec/scenario-001-xband.md) — the first vertical
  slice: a real aircraft trajectory to baseband IQ to a range-Doppler map, in three ambiguity variants
- [`spec/scenario-002-bistatic.md`](spec/scenario-002-bistatic.md) — the same path
  over a two-site geometry: the Raleigh-Durham illuminator received at the Duke Receiver
- [`spec/scenario-003-tracking.md`](spec/scenario-003-tracking.md) — the third
...
# 1. Describe the sensor: an X-band FMCW radar at a fixed geodetic site.
radar = rf.core.Radar(
    transmitter=rf.core.Transmitter(
        f0_hz=9.8e9,
        bandwidth_hz=2.0e6,
    ...
    receiver=rf.core.Receiver(sample_rate_hz=1.0e6, gain_rx_dbi=30.0, noise_figure_db=3.0),
    latitude_deg=35.0,
    longitude_deg=-78.0,
    altitude_m=60.0,
)

# 2. Place a point target and build its propagation path.
paths = rf.core.line_of_sight_paths(radar, range_m=12_000.0, radial_velocity_mps=-6.0, rcs_m2=10.0)

# 3. Synthesise complex baseband I/Q, shape (n_pulses, n_samples).
baseband_iq = rf.core.fmcw_deramp_baseband(paths, radar, n_pulses=256, rng=np.random.default_rng(0))

# 4. Process to a range-Doppler map and detect.
rd_map = rf.core.range_doppler_map(baseband_iq)
detected = rf.core.cfar_detect(np.abs(rd_map) ** 2, pfa=1e-4, n_train=8, n_guard=2)

# 5. Read the brightest cell back in physical units.
peak = np.unravel_index(np.abs(rd_map).argmax(), rd_map.shape)
range_axis_m = rf.core.range_bin_centers_m(
    rd_map.shape[1],
    radar.transmitter.bandwidth_hz,
    radar.transmitter.chirp_duration_s,
    radar.receiver.sample_rate_hz,
)
velocity_axis_mps = rf.core.doppler_bin_centers_mps(
    rd_map.shape[0], radar.transmitter.pulse_repetition_interval_s, radar.wavelength_m
)
print(range_axis_m[peak[1]], velocity_axis_mps[peak[0]])  # ~11992.0 -5.97
```

`fmcw_deramp_baseband` names the domain it returns: deramped FMCW baseband, one
complex sample per fast-time sample per chirp. Its pulse-train counterpart is
`pulsed_baseband`. There is no generic `simulate` method, and there is no
waveform object — the waveform is a field on `Transmitter`.

Run whole scenarios, rather than a single dwell, with `rf.pipelines.load_scenario`
and `scripts/run_scenario.py`.
